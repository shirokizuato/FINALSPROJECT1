...
#data directory to save all user lists
DATA_DIR = "user_data"
...
class PackingApp:
        ...
        with open(USER_FILE, "r") as f:
            for line in f:
                ...
                if u == username and p == password:
                    ...
                    #make user's list file if it doesn't exist
    ...
    def create_main_ui(self):
        ...
        #Button Frame with all action buttons
        button_frame = ctk.CTkFrame(self.root)
        button_frame.pack(pady=10, expand=True, fill="both", padx=20)
        ...
        remaining_buttons = [
            ("Add Item", self.add_item_popup),
            ("Delete Item", self.delete_item_popup)
        ]

        for text, command in remaining_buttons:
            btn = ctk.CTkButton(
                button_frame,
                text=text,
                command=command,
    ...
    def add_item_popup(self):
        ...
        def add():
            ...
            if item:
                self.textbox.configure(state="normal")  # Temporarily enable to add text
                current_text = self.textbox.get("1.0", END).strip()
                if current_text:
                    self.textbox.insert(END, f"\n{item}")
                else:
                    self.textbox.insert(END, item)
                self.textbox.configure(state="disabled")  # Disable after adding
                popup.destroy()
    ...
    def delete_item_popup(self):
        self.textbox.configure(state="normal")  # Temporarily enable to get text
        items = [item.strip() for item in self.textbox.get("1.0", END).splitlines() if item.strip()]
        self.textbox.configure(state="disabled")  # Disable again
        ...
        def delete_selected_items():
            ...
            self.textbox.configure(state="normal")  # Enable for modification
            self.textbox.delete("1.0", END)
            if remaining_items:
                self.textbox.insert(END, '\n'.join(remaining_items) + '\n')
            self.textbox.configure(state="disabled")  # Disable after modification
    ...
    def save_list(self):
        ...
        user_list_file = os.path.join(DATA_DIR, f"{self.current_user}_list.txt")
        self.textbox.configure(state="normal")  # Enable temporarily to get content
        with open(user_list_file, "w") as f:
            f.write(self.textbox.get("1.0", END).strip())
        self.textbox.configure(state="disabled")  # Disable after getting content
        ...
        if os.path.exists(user_list_file):
            with open(user_list_file, "r") as f:
                ...
                if content:  # Only show popup if there's actual content
                    ...
                    self.textbox.configure(state="disabled")  # Disable after loading
